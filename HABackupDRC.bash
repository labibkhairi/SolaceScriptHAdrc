...
docker ps -a
docker stop solace-backup-drc
docker rm -f solace-backup-drc
docker ps -a
...
--restart=always \
--hostname=solaceBackupDRC \
--add-host solace-backup-drc:10.17.101.53 \
--detach=true \
--memory-swap=-1 \
...
--env 'redundancy_matelink_connectvia=solace-primary-drc' \
...
--name=solace-backup-drc solace/solace-pubsub-standard:10.2.1.32
