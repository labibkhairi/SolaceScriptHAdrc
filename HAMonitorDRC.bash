...
docker ps -a
docker stop solace-monitoring-drc
docker rm -f solace-monitoring-drc
docker ps -a
...
--restart=always \
--add-host solace-monitoring-drc:10.17.101.6 \
--hostname=solaceMonitoringDRC \
--detach=true \
--memory-swap=-1 \
...
--name=solace-monitoring-drc solace/solace-pubsub-standard:10.2.1.32
